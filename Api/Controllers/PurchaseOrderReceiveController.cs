...
public class PurchaseOrderReceiveController : ERPApiController
{
    ...
    [ProducesResponseType(typeof(Response<PurchaseOrderReceiveHeaderDto>), 200)]
    [ProducesResponseType(400)]
    public async Task<ActionResult> GetDtoByPOId([FromQuery] int purchase_order_id)
    {
        var result = await _Module.GetDtoByPOId(purchase_order_id);

        // Removed so that the UI doesn't blow up on pruchase order receive page
        //if (!result.Success)
        //    return BadRequest(result);
