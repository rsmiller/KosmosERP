...
{
    Task<Response<CountryDto>> GetByISOAsync(string iso3);
}
...
public class CountryModule : BaseERPModule, ICountryModule
{
    ...
    public async Task<Response<CountryDto>> GetDtoByGuid(string guid)
    {
        throw new NotImplementedException();
    }
    
    public async Task<Response<CountryDto>> GetByISOAsync(string iso3)
    {
        var entity = await _Context.Countries.SingleOrDefaultAsync(m => m.iso3 == iso3);
        if (entity == null)
            return new Response<CountryDto>("Country not found", ResultCode.NotFound);
    ...
    public async Task<Response<List<CountryListDto>>> GlobalSearch(GlobalSearchFindCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<List<CountryListDto>>(validationResult.Exception, ResultCode.DataValidationError);
