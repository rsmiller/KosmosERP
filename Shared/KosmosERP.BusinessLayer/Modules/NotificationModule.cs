...
public interface INotificationModule
{
    ...
    Task<Response<NotificationDto>> GetDtoByGuid(string guid);
...
public class NotificationModule : Hub, INotificationModule
{
    ...
    public async Task<Response<NotificationDto>> GetDtoByGuid(string guid)
    {
        var entity = await _Context.Notifications.FirstOrDefaultAsync(c => c.guid == guid && !c.is_deleted);
        if (entity == null)
            return new Response<NotificationDto>("Notification not found", ResultCode.NotFound);

        var dto = await MapToDto(entity);
        return new Response<NotificationDto>(dto);
    }
