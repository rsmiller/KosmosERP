...
public class ProductModule : BaseERPModule, IProductModule
{
    ...
    public override Guid ModuleIdentifier => Guid.Parse("bc4b4287-e927-4b9a-b0cf-1174f0a13f39");
    ...
    public ProductModule(IBaseERPContext context, 
                            ...
                            ILogProviderFactory logProviderFactory) : base(context, logProviderFactory)
    {
        ...
        _MessagePublisherSettings = messagePublisherSettings;
    }

    public override void SeedPermissions()
    {
        var role = _Context.Roles.Any(m => m.name == "Product Administrators");

        if (role == false)
        {
            _Context.Roles.Add(CommonDataHelper<Role>.FillCommonFields(new Role()
            {
                name = "Product Administrators",
            }, 1));
    ...
    public async Task<Response<ProductDto>> GetDtoByGuid(string guid)
    {
        var entity = await _Context.Products.FirstOrDefaultAsync(c => c.guid == guid && !c.is_deleted);
        if (entity == null)
    ...
    public async Task<Response<ProductDto>> Edit(ProductEditCommand commandModel)
    {
        ...
        try
        {
            ...
            if (commandModel.manufacture_time_minutes.HasValue && existingEntity.manufacture_time_minutes != commandModel.manufacture_time_minutes)
                existingEntity.manufacture_time_minutes = commandModel.manufacture_time_minutes.Value;

            
            if (commandModel.required_stock_level.HasValue && existingEntity.required_stock_level != commandModel.required_stock_level)
                existingEntity.required_stock_level = commandModel.required_stock_level.Value;
            if (commandModel.required_reorder_level.HasValue && existingEntity.required_reorder_level != commandModel.required_reorder_level)
                existingEntity.required_reorder_level = commandModel.required_reorder_level.Value;
            if (commandModel.required_min_order.HasValue && existingEntity.required_min_order != commandModel.required_min_order)
                existingEntity.required_min_order = commandModel.required_min_order.Value;
            ...
            if (commandModel.is_sales_item.HasValue && existingEntity.is_sales_item != commandModel.is_sales_item)
                existingEntity.is_sales_item = commandModel.is_sales_item.Value;
            if (commandModel.is_manufactured.HasValue && existingEntity.is_manufactured != commandModel.is_manufactured)
                existingEntity.is_manufactured = commandModel.is_manufactured.Value;
    ...
    public async Task<Response<List<ProductListDto>>> GlobalSearch(GlobalSearchFindCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<List<ProductListDto>>(validationResult.Exception, ResultCode.DataValidationError);


        var response = new Response<List<ProductListDto>>();
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(commandModel.wildcard))
            {
                ...
                filter = filter.And(m =>
                    m.identifier1.ToLower().Contains(lower)
                    || m.identifier2.ToLower().Contains(lower)
                    || m.identifier3.ToLower().Contains(lower)
                    || m.category.ToLower().Contains(lower)
                    || m.product_name.ToLower().Contains(lower)
                    || m.product_class.ToLower().Contains(lower)
                    || m.guid.ToLower().Contains(lower)
                );
            }

            var results = _Context.Products.Where(filter);
    ...
    public async Task<ProductListDto> MapToListDto(Database.Models.Product databaseModel)
    {
        ...
        var category_val = await _KVMemoryService.GetKeyValue(databaseModel.category);

        if (category_val != null)
            dto.category_name = category_val.value;
    ...
    public async Task<ProductDto> MapToDto(Database.Models.Product databaseModel)
    {
        var dto = new ProductDto()
        {
            ...
            product_name = databaseModel.product_name,
            is_sales_item = databaseModel.is_sales_item,
            is_manufactured = databaseModel.is_manufactured,
            manufacture_time_minutes = databaseModel.manufacture_time_minutes,
            category = databaseModel.category,
            guid = databaseModel.guid,
            id = databaseModel.id,
            internal_description = databaseModel.internal_description,
    ...
    public Database.Models.Product MapToDatabaseModel(ProductDto dtoModel)
    {
        return new Database.Models.Product()
        {
            ...
            is_manufactured = dtoModel.is_manufactured,
            manufacture_time_minutes = dtoModel.manufacture_time_minutes,
    ...
    public Database.Models.Product MapToDatabaseModel(ProductCreateCommand createCommand)
    {
        return CommonDataHelper<Database.Models.Product>.FillCommonFields(new Database.Models.Product()
        {
            ...
            is_manufactured = createCommand.is_manufactured,
            manufacture_time_minutes = createCommand.manufacture_time_minutes,
    ...
    public ProductAttribute MapToAttributeDatabaseModel(ProductAttributeCreateCommand createCommand, string calling_user_id)
    {
