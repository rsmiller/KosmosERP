using Microsoft.AspNetCore.Cryptography.KeyDerivation;
using System.Security.Cryptography;
using KosmosERP.Database.Models;
using KosmosERP.Models.Helpers;
using KosmosERP.Models;
using KosmosERP.Models.Interfaces;
using KosmosERP.Module;
using KosmosERP.Database;
using Microsoft.EntityFrameworkCore;
using KosmosERP.BusinessLayer.Models.Module.User.Dto;
using KosmosERP.BusinessLayer.Models.Module.User.Command.Create;
using KosmosERP.BusinessLayer.Models.Module.User.Command.Delete;
using KosmosERP.BusinessLayer.Models.Module.User.Command.Edit;
using KosmosERP.BusinessLayer.Models.Module.User.Command.Find;
using KosmosERP.BusinessLayer.Helpers;
using KosmosERP.BusinessLayer.Models.Module;
using KosmosERP.BusinessLayer.Models.Module.User.ListProfiles;
using KosmosERP.BusinessLayer.Interfaces;
namespace KosmosERP.BusinessLayer.Modules;

public partial interface IUserModule : IERPModule<KosmosERP.Database.Models.User, UserDto, UserListDto, UserCreateCommand, UserEditCommand, UserDeleteCommand, UserFindCommand>, IBaseERPModule
{
    Task<Response<AuthenticatedUserDto>> Authenticate(AuthenticateCommand commandModel);
    Task<Response<UserDto>> CreateExtended(UserCreateCommand commandModel, string auth_token);
    Task<Response<UserPermissionsSet>> GetPermissionSet(int? user_id);
    Task<Response<UserPermissionsSet>> GetPermissionSet(string session_id);
    Task<Response<UserDto>> GetBySession(string session_id);
    Task<Response<List<UserDto>>> GetUsersByDepartment(string department_id);
    Task<Response<bool>> AssignUserRole(AssignUserRoleCommand command);
    Task<Response<List<RoleDto>>> GetRoles();
    Task<Response<List<UserAdminListDto>>> GetUsers();
    Task<Response<List<RolePermissionsDto>>> GetRolePermissions(int role_id);
    Task<Response<bool>> CreateNewRoleModulePermission(RoleModulePermissionCreateCommand commandModel);
    Task<Response<bool>> EditRoleModulePermission(RoleModulePermissionEditCommand commandModel);
    Task<Response<bool>> DeleteRoleModulePermission(ModulePermissionDeleteCommand commandModel);
    Task<Response<RoleDto>> CreateRole(RoleCreateCommand commandModel);
    Task<Response<bool>> DeleteRole(RoleDeleteCommand commandModel);
}

public partial class UserModule : BaseERPModule, IUserModule
{
    public override Guid ModuleIdentifier => Guid.Parse("b8b0d255-3901-4007-b9c7-b0678f89c955");
    public override string ModuleName => "Users";

    private IBaseERPContext _IContext;

    private IAuthenticationSettings _AuthenticationSettings;
    private IKeyValueModule _KVModule;
    private IAuthenticationProvider _AuthenticationProvider;

    public UserModule(IBaseERPContext context, IAuthenticationFactory authFactory, ILogProviderFactory logProviderFactory) : base(logProviderFactory)
    {
        _IContext = context;
        _AuthenticationProvider = authFactory.GetProvider();
    }

    public UserModule(IBaseERPContext context, 
                        IAuthenticationSettings authenticationSettings, 
                        IKeyValueModule keyValueModule, 
                        IAuthenticationFactory authFactory, 
                        ILogProviderFactory logProviderFactory) : base(logProviderFactory)
    {
        _IContext = context;
        _AuthenticationSettings = authenticationSettings;
        _KVModule = keyValueModule;
        _AuthenticationProvider = authFactory.GetProvider();
    }

    public override void SeedPermissions()
    {
        var admin_role = _IContext.Roles.Any(m => m.name == "Administrators");
        var system_user = _IContext.Users.Any(m => m.username == "system");

        if(admin_role == false)
        {
            _IContext.Roles.Add(CommonDataHelper<Role>.FillCommonFields(new Role()
            {
                name = "Administrators",
            }, 1));

            _IContext.SaveChanges();
        }

        if (system_user == false)
        {
            string new_password = Guid.NewGuid().ToString().Substring(0, 12);

            // Need to make this 128b happy
            int index = new_password.Select((c, i) => new { c, i }).FirstOrDefault(x => !char.IsDigit(x.c))?.i ?? 0;
            char capitalized = char.ToUpper(new_password[index]);
            new_password = new_password.Substring(0, index) + capitalized + new_password.Substring(index + 1);


            var password = this.GeneratePasword(new_password);

            Console.WriteLine("Administrator user generated with password: " + new_password);

            _IContext.Users.Add(CommonDataHelper<User>.FillCommonFields(new User()
            {
                username = "system",
                employee_number = "1",
                first_name = "System",
                last_name = "User",
                password = password.HashedPassword,
                password_salt = password.Salt,
                is_admin = true,
            }, 1));

            _IContext.SaveChanges();

            var role_id = _IContext.Roles.Where(m => m.name == "Administrators").Select(m => m.id).Single();
            var user_id = _IContext.Users.Where(m => m.username == "system").Select(m => m.id).Single();

            _IContext.UserRoles.Add(CommonDataHelper<UserRole>.FillCommonFields(new UserRole()
            {
                role_id = role_id,
                user_id = user_id,
            }, 1));

            _IContext.SaveChanges();
        }

        // Seed ModulePermissions for User module
        var existing_permissions = _IContext.ModulePermissions.Any(m => m.module_id == this.ModuleIdentifier.ToString());
        if (!existing_permissions)
        {
            var permissions = new List<ModulePermission>
            {
                CommonDataHelper<ModulePermission>.FillCommonFields(new ModulePermission()
                {
                    module_id = this.ModuleIdentifier.ToString(),
                    module_name = this.ModuleName,
                    permission_name = "Read Users",
                    internal_permission_name = "read_user",
                    read = true,
                    write = false,
                    edit = false,
                    delete = false,
                    is_active = true
                }, 1),
                CommonDataHelper<ModulePermission>.FillCommonFields(new ModulePermission()
                {
                    module_id = this.ModuleIdentifier.ToString(),
                    module_name = this.ModuleName,
                    permission_name = "Create Users",
                    internal_permission_name = "create_user",
                    read = false,
                    write = true,
                    edit = false,
                    delete = false,
                    is_active = true
                }, 1),
                CommonDataHelper<ModulePermission>.FillCommonFields(new ModulePermission()
                {
                    module_id = this.ModuleIdentifier.ToString(),
                    module_name = this.ModuleName,
                    permission_name = "Edit Users",
                    internal_permission_name = "edit_user",
                    read = false,
                    write = false,
                    edit = true,
                    delete = false,
                    is_active = true
                }, 1),
                CommonDataHelper<ModulePermission>.FillCommonFields(new ModulePermission()
                {
                    module_id = this.ModuleIdentifier.ToString(),
                    module_name = this.ModuleName,
                    permission_name = "Delete Users",
                    internal_permission_name = "delete_user",
                    read = false,
                    write = false,
                    edit = false,
                    delete = true,
                    is_active = true
                }, 1)
            };

            _IContext.ModulePermissions.AddRange(permissions);
            _IContext.SaveChanges();
        }
    }

    public async Task<Response<List<UserAdminListDto>>> GetUsers()
    {
        Response<List<UserAdminListDto>> response = new Response<List<UserAdminListDto>>();
        response.Data = new List<UserAdminListDto>();

        try
        {
            var module_response = await _KVModule.GetModuleInfo();

            if (!module_response.Success)
            {
                await LogError(50, this.GetType().Name, "GetUsers", new Exception("Could not load Module assemlby data"));
                response.SetException("Could not load Module assemlby data", ResultCode.Error);

                return response;
            }

            var results = await _IContext.Users.ToListAsync();

            foreach (var usr in results)
            {
                response.Data.Add(await this.MapToAdminListDto(usr, module_response.Data));
            }
        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "GetUsers", ex);
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }

    public async Task<Response<List<RoleDto>>> GetRoles()
    {
        Response<List<RoleDto>> response = new Response<List<RoleDto>>();
        response.Data = new List<RoleDto>();

        try
        {
            var module_response = await _KVModule.GetModuleInfo();

            if (!module_response.Success)
            {
                await LogError(50, this.GetType().Name, "GetRoles", new Exception("Could not load Module assemlby data"));
                response.SetException("Could not load Module assemlby data", ResultCode.Error);

                return response;
            }

            var roles = await _IContext.Roles.Where(m => m.is_deleted == false).OrderBy(m => m.name).ToListAsync();

            foreach (var role in roles)
            {
                var dto = new RoleDto()
                {
                    name = role.name,
                    role_id = role.id
                };

                var role_permissions = await _IContext.RolePermissions.Where(m => m.role_id == role.id && !m.is_deleted).ToListAsync();

                foreach (var role_permission in role.role_permissions)
                    dto.role_permissions.Add(await this.MapToPermissionDto(role_permission, module_response.Data));

                
                response.Data.Add(dto);
            }

        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "GetRoles", ex);
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }

    public async Task<Response<UserPermissionsSet>> GetPermissionSet(string session_id)
    {
        Response<UserPermissionsSet> response = new Response<UserPermissionsSet>();

        var result = await _IContext.UserSessionStates.FirstOrDefaultAsync(m => m.session_id == session_id);

        if (result == null || result.session_expires <= DateTime.UtcNow)
        {
            return new Response<UserPermissionsSet>("User session expired or does not exist", ResultCode.NotFound);
        }

        return await GetPermissionSet(result.user_id);
    }

    public async Task<Response<UserPermissionsSet>> GetPermissionSet(int user_id)
    {
        Response<UserPermissionsSet> response = new Response<UserPermissionsSet>();

        try
        {
            response.Data = new UserPermissionsSet()
            {
                user_id = user_id,
                permissions = new List<RolePermission>(),
            };


            var results = await (from ur in _IContext.UserRoles
                                 join u in _IContext.Users on ur.user_id equals u.id
                                 join r in _IContext.Roles on ur.role_id equals r.id
                                 join rp in _IContext.RolePermissions on r.id equals rp.role_id
                                 where ur.user_id == user_id
                                 select new { role_id = r.id, isAdmin = u.is_admin, permission = rp }).ToListAsync();


            foreach (var result in results)
                response.Data.permissions.Add(result.permission);

            if(results.Any())
                response.Data.is_admin = results.First().isAdmin;
        }
        catch (Exception ex)
        {
            await LogError(90, this.GetType().Name, "GetPermissionSet", ex);
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }

    public async Task<Response<UserDto>> GetBySession(string session_id)
    {
        Response<UserDto> response = new Response<UserDto>();

        try
        {
            var result = await _IContext.UserSessionStates.FirstOrDefaultAsync(m => m.session_id == session_id);

            if (result == null || result.session_expires <= DateTime.UtcNow)
            {
                response.SetException("User session expired or does not exist", ResultCode.NotFound);
            }
            else
            {
                var user = await this.GetDto(result.user_id);
                await _AuthenticationProvider.FindCreateOrUpdateUserSession(result.user_id);

                response.ResultCode = ResultCode.Okay;
                response.Data = user.Data;
            }
        }
        catch (Exception ex)
        {
            await LogError(90, this.GetType().Name, "GetBySession", ex);
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }

    public async Task<Response<List<UserDto>>> GetUsersByDepartment(string department_id)
    {
        Response<List<UserDto>> response = new Response<List<UserDto>>();
        response.Data = new List<UserDto>();

        try
        {
            var results = await _IContext.Users.Where(m => m.department == department_id
                                                        && m.is_deleted == false).ToListAsync();

            foreach (var user in results)
                response.Data.Add(await this.MapToDto(user));

        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "GetUsersByDepartment", ex);
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }

    public async Task<Response<List<RolePermissionsDto>>> GetRolePermissions(int role_id)
    {
        Response<List<RolePermissionsDto>> response = new Response<List<RolePermissionsDto>>();
        response.Data = new List<RolePermissionsDto>();

        try
        {
            var module_response = await _KVModule.GetModuleInfo();

            if (!module_response.Success)
            {
                await LogError(50, this.GetType().Name, "GetUsersByDepartment", new Exception("Could not load Module assemlby data"));
                response.SetException("Could not load Module assemlby data", ResultCode.Error);

                return response;
            }


            var results = await _IContext.RolePermissions.Where(m => m.role_id == role_id && m.is_deleted == false).ToListAsync();

            foreach (var perm in results)
                response.Data.Add(await this.MapToPermissionDto(perm, module_response.Data));

        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "GetUsersByDepartment", ex);
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }

    public async Task<Response<AuthenticatedUserDto>> Authenticate(AuthenticateCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<AuthenticatedUserDto>(validationResult.Exception, ResultCode.DataValidationError);

        try
        {
            var response = await _AuthenticationProvider.Authenticate(commandModel.username, commandModel.password);

            if (response.Success)
            {
                var dto = response.Data;
                var user = await this.GetDto(dto.id);

                if(!user.Success)
                    return new Response<AuthenticatedUserDto>(user.Exception, user.ResultCode);

                dto.user = user.Data;

                return new Response<AuthenticatedUserDto>(dto);
            }

            return new Response<AuthenticatedUserDto>("Credentials could not be authenticated", ResultCode.Invalid);
        }
        catch (Exception ex)
        {
            await LogError(20, this.GetType().Name, "Authenticate", ex);

            return new Response<AuthenticatedUserDto>(ex.Message, ResultCode.Error);
        }
    }

    public async Task<Response<UserDto>> GetDto(int object_id)
    {
        Response<UserDto> response = new Response<UserDto>();

        var result = await _IContext.Users.SingleOrDefaultAsync(m => m.id == object_id);
        if (result == null)
        {
            response.SetException("User not found", ResultCode.NotFound);
            return response;
        }

        response.Data = await this.MapToDto(result);
        return response;
    }

    public async Task<Response<UserDto>> GetDtoByGuid(string guid)
    {
        var entity = await _IContext.Users.FirstOrDefaultAsync(c => c.guid == guid && !c.is_deleted);
        if (entity == null)
            return new Response<UserDto>("User not found", ResultCode.NotFound);

        var dto = await MapToDto(entity);
        return new Response<UserDto>(dto);
    }

    public async Task<Response<bool>> AssignUserRole(AssignUserRoleCommand commandModel)
    {
        Response<bool> response = new Response<bool>();

        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<bool>(validationResult.Exception, ResultCode.DataValidationError);

        var user = await _IContext.Users.SingleOrDefaultAsync(m => m.id == commandModel.user_id);
        if (user == null)
            return new Response<bool>("User not found", ResultCode.NotFound);

        var permission = _IContext.Roles.SingleOrDefault(m => m.id == commandModel.role_id);
        if (permission == null)
            return new Response<bool>("Role not found", ResultCode.NotFound);

        var user_role = _IContext.UserRoles.SingleOrDefault(m => m.user_id == commandModel.user_id && m.role_id == commandModel.role_id);
        if (permission == null)
            return new Response<bool>("User already has role", ResultCode.AlreadyExists);


        await _IContext.UserRoles.AddAsync(CommonDataHelper<UserRole>.FillCommonFields(new UserRole()
        {
            user_id = commandModel.user_id,
            role_id = commandModel.role_id,
        }, commandModel.user_id));

        await _IContext.SaveChangesAsync();

        return new Response<bool>(true);
    }

    public async Task<Response<bool>> CreateNewRoleModulePermission(RoleModulePermissionCreateCommand commandModel)
    {
        if (commandModel == null)
            return new Response<bool>("Null", ResultCode.NullItemInput);

        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<bool>(validationResult.Exception, ResultCode.DataValidationError);

        try
        {
            var role_permission = CommonDataHelper<RolePermission>.FillCommonFields(new RolePermission()
            {
                role_id = commandModel.role_id,
                module_id = commandModel.module_id,
                read = commandModel.read,
                edit = commandModel.edit,
                write = commandModel.write,
                delete = commandModel.delete,

            }, commandModel.calling_user_id);


            await _IContext.RolePermissions.AddAsync(role_permission);
            await _IContext.SaveChangesAsync();

            return new Response<bool>(true);
        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "CreateNewRoleModulePermission", ex);
            return new Response<bool>(ex.Message, ResultCode.Error);
        }
    }

    public async Task<Response<RoleDto>> CreateRole(RoleCreateCommand commandModel)
    {
        if (commandModel == null)
            return new Response<RoleDto>("Null", ResultCode.NullItemInput);

        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<RoleDto>(validationResult.Exception, ResultCode.DataValidationError);


        try
        {

            var role_model = CommonDataHelper<Role>.FillCommonFields(new Role()
            {
                name = commandModel.role_name

            }, commandModel.calling_user_id);

            await _IContext.Roles.AddAsync(role_model);
            await _IContext.SaveChangesAsync();

            return new Response<RoleDto>(new RoleDto()
            {
                role_id = role_model.id,
                name = role_model.name,

            });
        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "CreateRole", ex);
            return new Response<RoleDto>(ex.Message, ResultCode.Error);
        }
    }

    public async Task<Response<UserDto>> CreateExtended(UserCreateCommand commandModel, string auth_token)
    {
        if (commandModel == null)
            return new Response<UserDto>("Null", ResultCode.NullItemInput);

        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<UserDto>(validationResult.Exception, ResultCode.DataValidationError);

        try
        {
            var alreadyExists = await _IContext.Users.FirstOrDefaultAsync(m => m.username.ToLower() == commandModel.username.ToLower());

            if (alreadyExists != null)
                return new Response<UserDto>("Username already in use", ResultCode.AlreadyExists);


            var response = await _AuthenticationProvider.CreateUser(commandModel, auth_token);

            var item = this.MapForCreate(commandModel);

            if(response.Success && response.Data != null && !String.IsNullOrEmpty(response.Data.id))
                item.external_id = response.Data.id;

            await _IContext.Users.AddAsync(item);
            await _IContext.SaveChangesAsync();

            var dto = await this.MapToDto(item);

            return new Response<UserDto>(dto);
        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "Create", ex);
            return new Response<UserDto>(ex.Message, ResultCode.Error);
        }
    }

    public async Task<Response<UserDto>> Create(UserCreateCommand commandModel)
    {
        throw new NotImplementedException();
    }

    public async Task<Response<bool>> EditRoleModulePermission(RoleModulePermissionEditCommand commandModel)
    {
        if (commandModel == null)
            return new Response<bool>("Null", ResultCode.NullItemInput);

        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<bool>(validationResult.Exception, ResultCode.DataValidationError);

        var existingEntity = await _IContext.RolePermissions.SingleOrDefaultAsync(m => m.id == commandModel.id);
        if (existingEntity == null)
            return new Response<bool>("ROle Permission not found", ResultCode.NotFound);


        if (!String.IsNullOrEmpty(commandModel.module_id) && existingEntity.module_id != commandModel.module_id)
            existingEntity.module_id = commandModel.module_id;


        if (commandModel.read.HasValue && commandModel.read != existingEntity.read)
            existingEntity.read = commandModel.read.Value;
        if (commandModel.write.HasValue && commandModel.write != existingEntity.write)
            existingEntity.write = commandModel.write.Value;
        if (commandModel.edit.HasValue && commandModel.edit != existingEntity.edit)
            existingEntity.edit = commandModel.edit.Value;
        if (commandModel.delete.HasValue && commandModel.delete != existingEntity.delete)
            existingEntity.delete = commandModel.delete.Value;

        
        try
        {
            existingEntity = CommonDataHelper<RolePermission>.FillUpdateFields(existingEntity, commandModel.calling_user_id);


            _IContext.RolePermissions.Update(existingEntity);
            await _IContext.SaveChangesAsync();

            return new Response<bool>(true);
        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "EditRoleModulePermission", ex);
            return new Response<bool>(ex.Message, ResultCode.Error);
        }
    }

    public async Task<Response<UserDto>> Edit(UserEditCommand commandModel)
    {
        if (commandModel == null)
            return new Response<UserDto>("Null", ResultCode.NullItemInput);

        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<UserDto>(validationResult.Exception, ResultCode.DataValidationError);

        var existingEntity = await _IContext.Users.SingleOrDefaultAsync(m => m.id == commandModel.id);
        if (existingEntity == null)
            return new Response<UserDto>("User not found", ResultCode.NotFound);


        var oldUneditedEntity = existingEntity;


        //var hashedPass = this.HashPasword(commandModel.password, existingEntity.password_salt);
        // UI will send 1234 as an unset password
        if (!String.IsNullOrEmpty(commandModel.password))//&& commandModel.password != "1234" && existingEntity.password != hashedPass)
        {
            // Generate new password
            var passwordInfo = GeneratePasword(commandModel.password);
            existingEntity.password = passwordInfo.HashedPassword;
            existingEntity.password_salt = passwordInfo.Salt;
        }


        if (!String.IsNullOrEmpty(commandModel.first_name) && existingEntity.first_name != commandModel.first_name)
            existingEntity.first_name = commandModel.first_name;
        if (!String.IsNullOrEmpty(commandModel.last_name) && existingEntity.last_name != commandModel.last_name)
            existingEntity.last_name = commandModel.last_name;
        if (!String.IsNullOrEmpty(commandModel.email) && existingEntity.email != commandModel.email)
            existingEntity.email = commandModel.email; // There is no email in emp

        if (!String.IsNullOrEmpty(commandModel.employee_number) && existingEntity.employee_number != commandModel.employee_number)
            existingEntity.employee_number = commandModel.employee_number;


        //Numbers
        if (!String.IsNullOrEmpty(commandModel.department) && existingEntity.department != commandModel.department)
            existingEntity.department = commandModel.department;

        if (commandModel.is_external_user.HasValue && existingEntity.is_external_user != commandModel.is_external_user)
            existingEntity.is_external_user = commandModel.is_external_user.Value;

        if (commandModel.is_disabled.HasValue && existingEntity.is_disabled != commandModel.is_disabled)
        {
            if (commandModel.is_disabled == false && existingEntity.is_disabled == true)
            {
                existingEntity.is_disabled = false;
                existingEntity.login_attempt = 0;
            }
            else
            {
                existingEntity.is_disabled = commandModel.is_disabled.Value;
            }
        }

        if (commandModel.is_deleted.HasValue && existingEntity.is_deleted != commandModel.is_deleted)
        {
            if (commandModel.is_deleted == true && existingEntity.is_deleted == false)
            {
                existingEntity.deleted_by = commandModel.calling_user_id;
                existingEntity.deleted_on = DateTime.UtcNow;
            }

            if (commandModel.is_deleted == false && existingEntity.is_deleted == true)
            {
                existingEntity.deleted_by = null;
                existingEntity.deleted_on = null;
            }

            existingEntity.is_deleted = commandModel.is_deleted.Value;
        }


        if (commandModel.is_admin.HasValue && commandModel.is_admin != existingEntity.is_admin)
            existingEntity.is_admin = commandModel.is_admin.Value;

        if (commandModel.is_guest.HasValue && commandModel.is_guest != existingEntity.is_guest)
            existingEntity.is_guest = commandModel.is_guest.Value;

        if (commandModel.is_management.HasValue && commandModel.is_management != existingEntity.is_management)
            existingEntity.is_management = commandModel.is_management.Value;

        try
        {
            existingEntity = CommonDataHelper<User>.FillUpdateFields(existingEntity, commandModel.calling_user_id);


            _IContext.Users.Update(existingEntity);
            await _IContext.SaveChangesAsync();

            return new Response<UserDto>(await this.MapToDto(existingEntity));
        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "Edit", ex);
            return new Response<UserDto>(ex.Message, ResultCode.Error);
        }
    }

    public async Task<Response<bool>> DeleteRoleModulePermission(ModulePermissionDeleteCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<bool>(validationResult.Exception, ResultCode.DataValidationError);

        var existingEntity = await _IContext.RolePermissions.SingleOrDefaultAsync(m => m.id == commandModel.id);
        if (existingEntity == null)
            return new Response<bool>("Module Permission not found", ResultCode.NotFound);


        existingEntity = CommonDataHelper<RolePermission>.FillDeleteFields(existingEntity, commandModel.calling_user_id);

        _IContext.RolePermissions.Update(existingEntity);
        await _IContext.SaveChangesAsync();

        return new Response<bool>(true);
    }

    public async Task<Response<UserDto>> Delete(UserDeleteCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<UserDto>(validationResult.Exception, ResultCode.DataValidationError);

        var existingEntity = await _IContext.Users.SingleOrDefaultAsync(m => m.id == commandModel.id);
        if (existingEntity == null)
            return new Response<UserDto>("User not found", ResultCode.NotFound);

        existingEntity = CommonDataHelper<User>.FillDeleteFields(existingEntity, commandModel.calling_user_id);

        _IContext.Users.Update(existingEntity);
        await _IContext.SaveChangesAsync();

        var dto = await MapToDto(existingEntity);
        return new Response<UserDto>(dto);
    }

    public async Task<Response<bool>> DeleteRole(RoleDeleteCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<bool>(validationResult.Exception, ResultCode.DataValidationError);


        var existingEntity = await _IContext.Roles.SingleOrDefaultAsync(m => m.id == commandModel.id);
        if (existingEntity == null)
            return new Response<bool>("Role not found", ResultCode.NotFound);

        existingEntity = CommonDataHelper<Role>.FillDeleteFields(existingEntity, commandModel.calling_user_id);

        _IContext.Roles.Update(existingEntity);
        await _IContext.SaveChangesAsync();

        return new Response<bool>(true);
    }

    public async Task<PagingResult<UserListDto>> Find(PagingSortingParameters parameters, UserFindCommand commandModel)
    {
        PagingResult<UserListDto> response = new PagingResult<UserListDto>();

        try
        {
            List<UserListDto> dtos = new List<UserListDto>();

            var results = _IContext.Users.Where(m => m.is_deleted == false &&
                                        (m.first_name.Contains(commandModel.wildcard)
                                        || m.last_name.Contains(commandModel.wildcard)
                                        || m.employee_number.Contains(commandModel.wildcard)
                                        || m.username.Contains(commandModel.wildcard)
                                        || m.email.Contains(commandModel.wildcard)));


            var sortedResults = await results.SortAndPageBy(parameters).ToListAsync();

            foreach (var result in results)
                dtos.Add(await this.MapToListDto(result));

            response.Data = dtos;
            response.TotalResultCount = results.Count();
        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "Find", ex);

            response.TotalResultCount = 0;
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }


    public async Task<Response<List<UserListDto>>> GlobalSearch(GlobalSearchFindCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<List<UserListDto>>(validationResult.Exception, ResultCode.DataValidationError);


        Response<List<UserListDto>> response = new Response<List<UserListDto>>();
        response.Data = new List<UserListDto>();

        try
        {
            string wildcard = commandModel.wildcard;
            var results = _IContext.Users.Where(m => m.is_deleted == false &&
                                        (m.first_name.Contains(wildcard)
                                        || m.last_name.Contains(wildcard)
                                        || m.employee_number.Contains(wildcard)
                                        || m.username.Contains(wildcard)
                                        || m.email.Contains(wildcard)));


            var sortedResults = await results.SortAndPageBy(commandModel.parameters).ToListAsync();

            foreach (var result in results)
                response.Data.Add(await this.MapToListDto(result));

        }
        catch (Exception ex)
        {
            await LogError(50, this.GetType().Name, "Find", ex);
            response.SetException(ex.Message, ResultCode.Error);
        }

        return response;
    }

    public async Task<UserDto> MapToDto(KosmosERP.Database.Models.User databaseModel)
    {
        return new UserDto
        {
            id = databaseModel.id,
            email = databaseModel.email,
            first_name = databaseModel.first_name,
            last_name = databaseModel.last_name,
            username = databaseModel.username,
            employee_number = databaseModel.employee_number,
            is_deleted = databaseModel.is_deleted,
            department = databaseModel.department,
            is_admin = databaseModel.is_admin,
            is_management = databaseModel.is_management,
            is_external_user = databaseModel.is_external_user,
            is_guest = databaseModel.is_guest,
            created_on = databaseModel.created_on,
            created_by = databaseModel.created_by,
            created_on_string = databaseModel.created_on_string,
            created_on_timezone = databaseModel.created_on_timezone,
            updated_on = databaseModel.updated_on,
            updated_by = databaseModel.updated_by,
            updated_on_string = databaseModel.updated_on_string,
            updated_on_timezone = databaseModel.updated_on_timezone,
        };
    }

    public async Task<UserListDto> MapToListDto(User databaseModel)
    {
        return new UserListDto
        {
            id = databaseModel.id,
            email = databaseModel.email,
            first_name = databaseModel.first_name,
            last_name = databaseModel.last_name,
            username = databaseModel.username,
            employee_number = databaseModel.employee_number,
            is_deleted = databaseModel.is_deleted,
            department = databaseModel.department,
            is_admin = databaseModel.is_admin,
            is_management = databaseModel.is_management,
            is_external_user = databaseModel.is_external_user,
            is_guest = databaseModel.is_guest,
            created_on = databaseModel.created_on,
            created_on_string = databaseModel.created_on_string,
            created_on_timezone = databaseModel.created_on_timezone,
            updated_on = databaseModel.updated_on,
            updated_by = databaseModel.updated_by,
            updated_on_string = databaseModel.updated_on_string,
            updated_on_timezone = databaseModel.updated_on_timezone,
        };
    }

    public async Task<UserAdminListDto> MapToAdminListDto(User databaseModel, List<ModuleObjectDto> module_data)
    {
        var dto = new UserAdminListDto
        {
            id = databaseModel.id,
            email = databaseModel.email,
            first_name = databaseModel.first_name,
            last_name = databaseModel.last_name,
            username = databaseModel.username,
            employee_number = databaseModel.employee_number,
            is_deleted = databaseModel.is_deleted,
            department = databaseModel.department,
            is_admin = databaseModel.is_admin,
            is_management = databaseModel.is_management,
            is_external_user = databaseModel.is_external_user,
            is_guest = databaseModel.is_guest,
            created_by = databaseModel.created_by,
            created_on = databaseModel.created_on,
            created_on_string = databaseModel.created_on_string,
            created_on_timezone = databaseModel.created_on_timezone,
            updated_on = databaseModel.updated_on,
            updated_by = databaseModel.updated_by,
            updated_on_string = databaseModel.updated_on_string,
            updated_on_timezone = databaseModel.updated_on_timezone,
        };

        //UserRoleDto


        var associated_user_roles = await (from r in _IContext.Roles
                                           join ur in _IContext.UserRoles on r.id equals ur.role_id
                                           where ur.user_id == databaseModel.id
                                           && !ur.is_deleted
                                           select new { role = r, user_role = ur }).ToListAsync();
                                           
        foreach (var user_role in associated_user_roles)
        {

            var user_role_dto = new UserRoleDto()
            {
                id = user_role.user_role.id,
                role_id = user_role.user_role.role_id,
                role_name = user_role.role.name
            };

            var role_permissions = await _IContext.RolePermissions.Where(m => m.role_id == user_role.role.id).ToListAsync();

            foreach (var role_perm in role_permissions)
            {
                user_role_dto.permissions.Add(await this.MapToPermissionDto(role_perm, module_data));
            }

            dto.user_roles.Add(user_role_dto);
        }

        return dto;
    }

    public async Task<RolePermissionsDto> MapToPermissionDto(RolePermission databaseModel, List<ModuleObjectDto> module_data)
    {
        
        var dto = new RolePermissionsDto()
        {
            role_id = databaseModel.role_id,
            module_id = databaseModel.module_id,
            read = databaseModel.read,
            edit = databaseModel.edit,
            write = databaseModel.write,
            delete = databaseModel.delete,
            requires_admin = databaseModel.requires_admin,
            requires_guest = databaseModel.requires_guest,
            requires_management = databaseModel.requires_management,
            id = databaseModel.id,
            created_on = databaseModel.created_on,
            created_on_string = databaseModel.created_on_string,
            created_on_timezone = databaseModel.created_on_timezone,
            updated_on = databaseModel.updated_on,
            updated_by = databaseModel.updated_by,
            updated_on_string = databaseModel.updated_on_string,
            updated_on_timezone = databaseModel.updated_on_timezone,
        };

        var associated_module = module_data.Where(m => m.module_id == databaseModel.module_id).FirstOrDefault();
        if (associated_module != null)
            dto.module_name = associated_module.module_name;


        return dto;
    }

    public KosmosERP.Database.Models.User MapToDatabaseModel(UserDto dtoModel)
    {
        throw new NotImplementedException();
    }

    private KosmosERP.Database.Models.User MapForCreate(UserCreateCommand createCommandModel)
    {
        var now = DateTime.UtcNow;

        var user = CommonDataHelper<KosmosERP.Database.Models.User>.FillCommonFields(new KosmosERP.Database.Models.User()
        {
            email = createCommandModel.email,
            first_name = createCommandModel.first_name,
            last_name = createCommandModel.last_name,
            username = createCommandModel.username,
            employee_number = createCommandModel.employee_number,
            is_deleted = createCommandModel.is_deleted,
            department = createCommandModel.department,
            is_admin = createCommandModel.is_admin,
            is_management = createCommandModel.is_management,
            is_external_user = createCommandModel.is_external_user,
            is_guest = createCommandModel.is_guest,

        }, createCommandModel.calling_user_id);

        var passwordInfo = GeneratePasword(createCommandModel.password);

        user.password = passwordInfo.HashedPassword;
        user.password_salt = passwordInfo.Salt;

        return user;
    }

    private UserPassword GeneratePasword(string password)
    {
        var newSalt = GenerateSalt();
        var hashedPassword = Convert.ToBase64String(KeyDerivation.Pbkdf2(
        password: password,
        salt: newSalt,
        prf: KeyDerivationPrf.HMACSHA1,
        iterationCount: 10000,
        numBytesRequested: 256 / 8));

        return new UserPassword()
        {
            Salt = Convert.ToBase64String(newSalt),
            HashedPassword = hashedPassword
        };
    }

    private byte[] GenerateSalt()
    {
        var bytes = new byte[128 / 8];
        RandomNumberGenerator.Fill(bytes);

        return bytes;
    }

    public KosmosERP.Database.Models.User? Get(int object_id)
    {
        return _IContext.Users.Single(m => m.id == object_id);
    }

    public async Task<KosmosERP.Database.Models.User?> GetAsync(int object_id)
    {
        return await _IContext.Users.SingleAsync(m => m.id == object_id);
    }

    public Task<Response<UserPermissionsSet>> GetPermissionSet(int? user_id)
    {
        throw new NotImplementedException();
    }


    private class UserPassword
    {
        public string HashedPassword { get; set; } = "";
        public string Salt { get; set; } = "";
    }
}
