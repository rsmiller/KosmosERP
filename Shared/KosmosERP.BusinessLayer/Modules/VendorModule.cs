...
public class VendorModule : BaseERPModule, IVendorModule
{
    ...
    {
        var role = _Context.Roles.Any(m => m.name == "Vendor Administrators");
    ...
    public async Task<Response<VendorDto>> GetDtoByGuid(string guid)
    {
        var entity = await _Context.Vendors.FirstOrDefaultAsync(c => c.guid == guid && !c.is_deleted);
        if (entity == null)
            return new Response<VendorDto>("Vendor not found", ResultCode.NotFound);
    ...
    public async Task<Response<VendorDto>> Edit(VendorEditCommand commandModel)
    {
        ...
        if (commandModel.address != null)
        {
            var edit_address_response = await _AddressModule.Edit(commandModel.address);
            if (!edit_address_response.Success)
            {
                var tdto = await MapToDto(existingEntity);
                var response = new Response<VendorDto>("Could not save address", ResultCode.Error);
                response.Data = tdto;
    ...
    public async Task<Response<List<VendorListDto>>> GlobalSearch(GlobalSearchFindCommand commandModel)
    {
        var validationResult = ModelValidationHelper.ValidateModel(commandModel);
        if (!validationResult.Success)
            return new Response<List<VendorListDto>>(validationResult.Exception, ResultCode.DataValidationError);
        ...
        try
        {
            var filter = PredicateBuilder.True<Vendor>();
            ...
            {
                filter = filter.And(m => m.vendor_number == parsed_num);
            }
            else
            {
            ...
            var results = _Context.Vendors.Where(filter);
