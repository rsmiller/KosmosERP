...
public class ShipmentHeaderConfiguration : BaseConfiguration<ShipmentHeader>
{
    public override void Configure(EntityTypeBuilder<ShipmentHeader> builder)
    {
        ...
        builder.HasIndex(x => new { x.order_header_id, x.is_canceled });
        builder.HasIndex(x => new { x.order_header_id, x.is_deleted });
        builder.HasIndex(x => new { x.order_header_id, x.is_complete });
