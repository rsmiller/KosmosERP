...
public class DatabaseTests
{
    ...
    public async Task RolesAndPermissions_ReadEdit()
    {
        ...
        var can_read = role.role_permissions.Where(m => m.read == true).Any();
        var can_edit = role.role_permissions.Where(m => m.edit == true).Any();
        var can_delete = role.role_permissions.Where(m => m.delete == true).Any();
        var can_write = role.role_permissions.Where(m => m.write == true).Any();


        Assert.That(can_read, Is.True);
        Assert.That(can_edit, Is.True);
        Assert.That(can_delete, Is.False);
        Assert.That(can_write, Is.False);
    }
    ...
    public async Task RolesAndPermissions_WriteDelete()
    {
        ...
        Assert.That(can_read, Is.False);
        Assert.That(can_edit, Is.False);
        Assert.That(can_delete, Is.True);
        Assert.That(can_write, Is.True);
    }
    ...
    public async Task Orders()
    {
        ...
        var order_model = CommonDataHelper<OrderHeader>.FillCommonFields(new OrderHeader()
        {
            ...
            pay_method = "payment_method_card",
    ...
    public async Task ARInvoices()
    {
        ...
        Assert.That(invoice_header_model.ar_invoice_lines[0].product, Is.Not.Null);
        Assert.That(invoice_header_model.ar_invoice_lines[0].order_line, Is.Not.Null);
    }
    ...
    public async Task Shipments()
    {
        ...
        Assert.That(shipment_header_model.address, Is.Not.Null);
        Assert.That(shipment_header_model.shipment_lines.Count() == 1);
        Assert.That(shipment_header_model.shipment_lines[0].order_line, Is.Not.Null);
    }
    ...
    public async Task PurchaseOrders()
    {
        ...
        Assert.That(purchase_order_model.vendor, Is.Not.Null);
        Assert.That(purchase_order_model.purchase_order_lines.Count() == 1);
        Assert.That(purchase_order_model.purchase_order_lines[0].product, Is.Not.Null);
    }
    ...
    public async Task PurchaseOrderReceive()
    {
        ...
        Assert.That(purchase_order_receive_model.purchase_order, Is.Not.Null);
        Assert.That(purchase_order_receive_model.purchase_order_receive_lines.Count() == 1);
        Assert.That(purchase_order_receive_model.purchase_order_receive_lines[0].purchase_order_line, Is.Not.Null);
    }
    ...
    public async Task Opportunities()
    {
        ...
        Assert.That(opportunity_model.customer, Is.Not.Null);
        Assert.That(opportunity_model.contact, Is.Not.Null);
